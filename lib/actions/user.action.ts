...
export async function getUserQuestions(params: GetUserStatsParams) {
  try {
    ...
    const userQuestions = await Question.find({ author: userId })
      ...
      const hasNextQuestions = totalQuestions > skipAmount + userQuestions.length;

    return { totalQuestions, questions: userQuestions, hasNextQuestions };
  } catch (error) {
    console.log(error);
    throw error;
  }
...
export async function getUserAnswers(params: GetUserStatsParams) {
  try {
    ...
    const userAnswers = await Answer.find({ author: userId })
      ...
      const hasNextAnswers = totalAnswers > skipAmount + userAnswers.length;

    return { totalAnswers, answers: userAnswers, hasNextAnswers };
  } catch (error) {
    console.log(error);
    throw error;
  }
